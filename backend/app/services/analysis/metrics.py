...
def _analyze_player(replay: ParsedReplay, number: int) -> PlayerAnalysis:
    ...
    for age in ("feudal", "castle", "imperial"):
        m[f"{age}_time"] = Metric(
            ...
            value=ages.get(age),
            unit="ms",
            availability=Availability.OBSERVED if age in ages else Availability.UNAVAILABLE,
    ...
    if samples:
        ...
        m["time_floating"] = Metric(
            "time_floating",
            f"Time above {FLOAT_THRESHOLD} banked",
            over,
            "ms",
            Availability.INFERRED,
    ...
    if queues:
        ...
        m["max_production_gap"] = Metric(
            "max_production_gap",
            "Longest gap between villager queues",
            _max_gap(villager_queues),
            "ms",
...
def _time_above(samples: list[ResourceSample], threshold: int) -> int:
...
def _float_by_age(
    samples: list[ResourceSample], ages: dict[str, int], duration_ms: int
) -> dict[str, int]:
...
def _max_gap(queue_commands: list[Command]) -> int | None:
...
def _classify_opening(builds: list[Command], ages: dict[str, int]) -> tuple[str, int | None]:
